  ...
  return (
    <main className="flex min-h-dvh items-center justify-center p-6">
      <div className="w-full max-w-sm rounded-(--radius-card) border border-line bg-surface p-8 text-center shadow-(--shadow-card)">
        ...
        <h1 className="text-2xl font-extrabold tracking-tight">Mend</h1>
        <p className="mt-1 text-sm text-muted">
          Injury recovery tracker with real-time sync
        </p>
        <button
          type="button"
          onClick={() => void signIn()}
        ...
        {authError ? (
          ...
          <p className="mt-4 text-xs text-muted">
            Sign in to share the recovery record across devices
          </p>
        )}
      </div>
    </main>
